...
async function placeOrder(userInfo) {
  ...
  if (!validateOrderUser(userInfo).isValid) {
    return {
      success: false,
      errors: validateOrderUser(userInfo).errors
    };
  }
  try {
...
function displayOrders(orders) {
  ...
  if (orders.length === 0) {
    ...
    orders.forEach((order, index) => {
      const fOrder = formatOrder(order);
      console.log(`訂單 ${index + 1}`);
      console.log('----------------------------------------');
      console.log(`訂單編號：${fOrder.id}`);
      console.log(`顧客姓名：${fOrder.user.name}`);
      console.log(`聯絡電話：${fOrder.user.tel}`);
      console.log(`寄送地址：${fOrder.user.address}`);
      console.log(`付款方式：${fOrder.user.payment}`);
      console.log(`訂單金額：${fOrder.totalFormatted}`);
      console.log(`付款狀態：${fOrder.paidText}`);
      console.log(`建立時間：${fOrder.createdAt} (${fOrder.daysAgo})`);
      console.log('----------------------------------------');
      console.log('商品明細：');
      fOrder.products.forEach(product => {
