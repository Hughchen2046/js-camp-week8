...
async function addProductToCart(productId, quantity) {
  ...
  if (!validateCartQuantity(quantity).isValid) {
    return {
      success: false,
      error: validateCartQuantity(quantity).error
    };
  }
  try {
...
async function getCartTotal() {
  ...
  const cartsData = await fetchCart();
  return {
    total: cartsData.total,
    finalTotal: cartsData.finalTotal,
    itemCount: cartsData.carts.length
  };
}
...
function displayCart(cart) {
  ...
  if (cart.carts.length === 0) {
    ...
    cart.carts.forEach((item, index) => {
      console.log(`${index + 1}. 產品名稱：${item.product.title}`);
